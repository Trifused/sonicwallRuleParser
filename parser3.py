...
# Updated to run with Python3 with 2to3.exe
...
for line in decoded_data:
    ...
    if re.match('^(iface|interface)', line):
        if re.match('^iface_ifnum_', line):
            ...
            ifaceName = urllib.parse.unquote(ifaceName)
            ...
            if interfaceZone:
                interfaceZone = urllib.parse.unquote(interfaceZone)
            else: 
            ...
            if ifaceComment:
                ifaceComment = urllib.parse.unquote(ifaceComment)
            else:
    ...
    if re.match('^natPolicy', line):
        ...
        if natRuleID and natOrigSrc and natOrigDest and natOrigService and natTransSrc and natTransDest and natTransService and natSrcInterface and natDestInterface and natReflexive and natComment and natStatus:
            ...
            natRule= {
                "natRuleID": natRuleID,
                "natOrigSrc": urllib.parse.unquote(natOrigSrc),
                "natOrigDest": urllib.parse.unquote(natOrigDest),
                "natOrigService": urllib.parse.unquote(natOrigService),
                "natTransSrc": urllib.parse.unquote(natTransSrc),
                "natTransDest": urllib.parse.unquote(natTransDest),
                "natTransService": urllib.parse.unquote(natTransService),
                "natSrcInterface": urllib.parse.unquote(natSrcInterface),
                "natDestInterface": urllib.parse.unquote(natDestInterface),
                "natSrcZone": urllib.parse.unquote(natSrcZone),
                "natDestZone": urllib.parse.unquote(natDestZone),
                "natReflexive": urllib.parse.unquote(natReflexive),
